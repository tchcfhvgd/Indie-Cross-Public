...
class AssetManager extends EventDispatcher
{
	...
	private function enqueueSingle(url:String, name:String = null /*, options : TextureOptions = null*/):String
	{
		...
		else
		{
			...
			if (assetReference.extension == "png")
				assetReference.name = StringTools.replace(assetReference.name, AnimationAtlasFactory.SPRITEMAP_SUFFIX, "");
		}
	...
	private function loadSingle():Void
	{
		...
		else
		{
			...
			else if (assetReference.extension == "json")
				Assets.loadText(assetReference.url).onComplete(onSingleComplete).onError(onErrorCallback).onProgress(onSingleProgress);
			else
				Assets.loadBitmapData(assetReference.url)
					.onComplete(onSingleComplete)
					.onError(onErrorCallback)
					.onProgress(onSingleProgress);
		};
	...
	public function getAssetNames(assetType:String, prefix:String = "", recursive:Bool = true, out:Array<String> = null):Array<String>
	{
