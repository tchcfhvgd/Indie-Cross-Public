...
class SubTexture extends BitmapData
{
	...
	public function new(parent:BitmapData, region:Rectangle = null, ownsParent:Bool = false, frame:Rectangle = null, rotated:Bool = false,
			scaleModifier:Float = 1)
	{
	...
	private function setTo(parent:BitmapData, region:Rectangle = null, ownsParent:Bool = false, frame:Rectangle = null, rotated:Bool = false,
			scaleModifier:Float = 1):Void
	{
		if (_region == null)
		{
		...
		if (Capabilities.isDebugger
			&& _frame != null
			&& (_frame.x > 0 || _frame.y > 0 || _frame.right + E < _width || _frame.bottom + E < _height))
		{
	...
	private function updateMatrices():Void
	{
		...
		while (texture != null)
		{
			...
			texture = try cast(texture.parent, SubTexture)
