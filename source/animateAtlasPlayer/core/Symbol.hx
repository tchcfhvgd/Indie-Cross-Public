...
class Symbol extends DisplayObjectContainer
{
	...
	public function nextFrame_MovieClips():Void
	{
		...
		for (l in 0..._numLayers)
		{
			...
			{
				(try cast(layer.getChildAt(e), Symbol)
				catch (e:Dynamic) null).nextFrame_MovieClips();
			}
		}
	}
	...
	private function updateLayer(layerIndex:Int):Void
	{
		...
		for (i in 0...numElements)
		{
			...
			oldSymbol = (layer.numChildren > i) ? try
				cast(layer.getChildAt(i), Symbol)
			catch (e:Dynamic)
				null : null;
			...
			if (layer.name.indexOf(Animation.ITEM_PREFIX) != 0)
				newSymbol.setBitmap(elementData.bitmap);
			else
			{
				var lAnim:Animation = getAnimation(layer);
				if (lAnim != null)
					newSymbol.setItem(lAnim.getItem(layer.name));
			}
	...
	private static function getAnimation(pSymbol:DisplayObjectContainer):Animation
	{
		if (pSymbol == null)
			return null;
		if (Std.isOfType(pSymbol, Animation))
			return cast(pSymbol, Animation);
		else
			return getAnimation(pSymbol.parent);
	}
	...
	public function createLayers():Void
	{
		...
		for (maskedLayer in maskedLayers.keys())
			maskedLayer.mask = _layers.getChildByName(maskedLayers[maskedLayer]);
	}
