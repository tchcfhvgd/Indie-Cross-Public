...
class Animation extends DisplayObjectContainer
{
	...
	{
		currentFrame = (Std.isOfType(indexOrLabel, String)) ? _symbol.getFrame(Std.string(indexOrLabel)) : Std.int(indexOrLabel);
	}
	...
	{
		if (swapName.indexOf(ITEM_PREFIX) != 0)
			throw new Error("Item prefix (" + ITEM_PREFIX + ") missing.");
		if (pItem == null)
			items[swapName] = new Sprite();
		else
			items[swapName] = pItem;
	...
	private function setItem(itemName:String, symbol:DisplayObjectContainer, item:DisplayObject = null):Void
	{
		...
		{
			if (!Std.isOfType(symbol.getChildAt(i), DisplayObjectContainer))
				continue;
			child = cast(symbol.getChildAt(i), DisplayObjectContainer);
			if (child.name.indexOf(itemName) == 0 && child.numChildren > 0)
				cast(child.getChildAt(0), Symbol).setItem(item);
			else
				setItem(itemName, cast(child, DisplayObjectContainer), item);
		}
	}
}
