...
class ModchartState
{
	...
	{
		#if cpp // I didn't write this lol
	...
	function new()
	{
		...
		setVar("scrollspeed", FlxG.save.data.scrollSpeed != 1 ? FlxG.save.data.scrollSpeed : PlayState.songScrollSpeed);
		...
		for (i in 0...PlayState.instance.strumLineNotes.length)
		{
			...
			trace(PlayState.instance.strumLineNotes.members[i].x
				+ " "
				+ PlayState.instance.strumLineNotes.members[i].y + " " + PlayState.instance.strumLineNotes.members[i].angle + " | strum" + i);
		...
		{
			var member = PlayState.instance.altStrumLineNotes.members[i];
			trace(PlayState.instance.altStrumLineNotes.members[i].x
				+ " "
				+ PlayState.instance.altStrumLineNotes.members[i].y + " " + PlayState.instance.altStrumLineNotes.members[i].angle + " | strum" + i);
			setVar("darkStrum" + i + "X", Math.floor(member.x));
			setVar("darkStrum" + i + "Y", Math.floor(member.y));
			setVar("darkStrum" + i + "Angle", Math.floor(member.angle));
			trace("Adding strum" + i);
		}
		#end
	}
